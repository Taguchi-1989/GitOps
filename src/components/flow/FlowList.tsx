    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${layerColors[flow.layer] || 'bg-gray-100 text-gray-700'}`}>
              {flow.layer}
            </span>
          </div>
      ...
      <div className="mt-3 flex items-center gap-4 text-xs text-gray-500">
        ...
        <span className="ml-auto">
          Updated: {new Date(flow.updatedAt).toLocaleDateString('ja-JP')}
        </span>
      </div>
    </Link>
  );
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <div className="text-gray-500">No flows found</div>
            <p className="text-sm text-gray-400 mt-1">
              Add YAML files to spec/flows/ to get started
            </p>
          </div>
        ) : (
          flows.map(flow => (
