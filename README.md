...
2. **AIの利用を「統治」する（ガバナンス・ハーネス）**
   ...
   - **監査ログ**: すべての判定をアプリ層で追記専用ログに記録（ポリシー版・時刻・ハッシュ付き）
...
npm install
cp .env.example .env
# .env の AUTH_SECRET と FLOWOPS_ADMIN_PASSWORD を安全な値に変更
# （LLM を試す時だけ LLM_API_KEY も設定）
npm run db:push                      # SQLite DB を作成
npm run db:seed                      # 初期データ投入
npm run dev                          # → http://localhost:3000
```

> FlowOps MVP の正本DBはローカル・CI・Dockerのすべてで **SQLite** です。`DATABASE_URL` は
> `file:` URL だけを受け付けます。`start.bat` は開発用の `.env` を自動生成します。

- ログイン: `.env` の `FLOWOPS_ADMIN_EMAIL` / `FLOWOPS_ADMIN_PASSWORD`
  （`start.bat` の自動生成時のみ `admin@flowops.local` / `admin`）
...
| **監査ログ**         | 全判定をアプリ層で追記専用記録。コンテンツハッシュ・ポリシー版・重大度層を保持         | `src/core/audit/`    |
...
| Database           | SQLite（MVPの正本。ローカル / CI / Docker共通）+ Prisma ORM |
...
**前提条件**: Node.js >= 20.19（Node.js 22 LTS推奨）だけ（Docker も Git も不要）
...
> PostgreSQL URL は起動時に拒否されます。将来移行する場合は Prisma schema・マイグレーション・
> 実行アダプタをまとめて切り替えます。
...
```bash
cp .env.example .env
# .env で AUTH_SECRET / FLOWOPS_ADMIN_PASSWORD / OPENAI_API_KEY 等を設定
...
| 変数                      | 必須        | 説明                                                  |
| ------------------------- | ----------- | ----------------------------------------------------- |
| `DATABASE_URL`            | Yes         | SQLiteパス（例: `file:./prisma/dev.db`）              |
| `AUTH_SECRET`             | Yes         | 32文字以上のランダムな認証秘密                        |
| `FLOWOPS_ADMIN_PASSWORD`  | Seed/Docker | 初期管理者のパスワード。Dockerでは必須                |
| `AUTH_DISABLED`           | No          | 開発時だけ認証を迂回。productionでは常に無効          |
...
| `AIMS_LLM_REVIEWERS`      | No          | AIMS複数LLM構成のJSON。キーではなく`apiKeyEnv`を指定  |
| `AIMS_REVIEW_CHUNK_CHARS` | No          | AIMSレビュー1チャンクの文字数（既定16000）            |
| `AIMS_MAX_REVIEW_CHUNKS`  | No          | AIMSレビューの最大チャンク数（既定6）                 |
...
npm run validate:flows # 全YAMLフローの辞書・構造整合性検証
...
> SQLite MVPでは追記専用性をアプリケーション境界で強制し、ハッシュで内容同一性を確認します。
> DB管理者まで含む物理的な改竄不能性は保証しません。本番でその保証が必要になった時点で、
> 権限分離・UPDATE/DELETE禁止・外部保管を含む永続層へ移行します。
...
- start/end ノードの存在確認
- 孤立ノード・到達不能パスの警告
- decision ノードの既定経路（判定不能時は安全側へ）

`npm run validate:flows` は全フローと辞書の整合性を検査し、CIでも実行されます。
...
| GET              | `/api/audit/export`                  | 監査ログCSV/JSON出力            |
| GET/POST         | `/api/workflows`                     | 実行一覧 / 実行開始             |
| POST             | `/api/workflows/:id/approve`         | 人手判断の記録                  |
| GET              | `/api/tasks`                         | マイクロタスク一覧              |
| GET/POST         | `/api/data-objects`                  | データ契約一覧 / 作成           |
| GET              | `/api/governance/trace/:traceId`     | ガバナンス判定追跡              |
| GET/POST         | `/api/aims/evidence`                 | AIMS証拠一覧 / 過去テキスト取込 |
| GET              | `/api/aims/evidence/:id`             | 原文・レビュー履歴              |
| POST             | `/api/aims/evidence/:id/reviews`     | 複数LLMレビューと統合           |
| GET              | `/api/aims/reviews/:id`              | モデル別・統合結果              |
| POST             | `/api/aims/reviews/:id/decision`     | 理由必須の人判断                |
| POST             | `/api/dexpi/import`                  | JSON/Mermaid/DeXPI XML取込      |
| POST             | `/api/dexpi/export`                  | JSON/Mermaid/DeXPI XML出力      |
| POST             | `/api/bpmn/import`                   | JSON/Mermaid/BPMN 2.0 XML取込   |
| POST             | `/api/bpmn/export`                   | JSON/Mermaid/BPMN/LLM出力       |
| GET              | `/api/openapi`                       | OpenAPI 3.0仕様                 |
...
npm run validate:flows # 全フローの構造・辞書整合性
```
...
| `docs/flow-design-guide.md`             | フロー定義・辞書ID・分岐条件・安全側既定経路の設計規約 |
| `docs/start.md`                         | システム要件定義書 v1.0                                |
| `docs/start.add.md`                     | レビュー追補提案（非機能要件, セキュリティ, 監査等）   |
| `docs/governance-manufacturing-todo.md` | 製造業ガバナンスの残課題                               |
...
| `docs/aims/llm-evidence-review.md`                    | 過去資料の正本化・複数LLMレビュー・人判断の運用仕様        |
| `docs/dexpi/README.md`                                | DeXPI 2.0 JSON正本・Mermaid・XML交換の設計と利用方法       |
| `docs/bpmn/README.md`                                 | BPMN 2.0.2 JSON正本・Mermaid・XML・LLM連携の設計と利用方法 |
